...
// страницу, чтобы избежать ошибки
        if ($page<1) {
            $page=1;
            header("Location: /admin/".$part_url."?page=1");
            exit();
        }
...
//код для редагування статті
        ...
        if (isset($_POST['save'])) {
            ...
            mysqli_query($db, "UPDATE $table SET title='$title', text='$text', author='$author' WHERE id='$id'");

            header("Location: /admin/".$part_url);
            exit();
        }
                ...
                while ($row = mysqli_fetch_array($result1)) {
                    ...
                    <div class="sidebar2">
                        ...
                        <div class="bottom_sidebar">        
                            ...
                            <a class="a-left delete" href="<?php echo base_url .$part_url .'?delete&id='.$row['id']; ?>"><i class="fas fa-trash-alt"></i>Видалити</a>
                            <a class="a-left edit" href="<?php echo base_url .$part_url .'?edit&id='.$row['id']; ?>"><i class="fas fa-pencil-alt"></i>Редагувати</a>
                        </div>
                    </div>
...
// Начальное число $j в нашем случае должно равнятся единице, а не нулю
    ...
    // уменьшаем его на единицу и получаем ссылку на последнюю страницу
            ...
            if (isset($_GET['articles'])) {
                ...
                <div class="sidebar2">
                    <a class="a-left delete" href="<?php echo base_url .$part_url .'?delete&id='.$roww['id']; ?>"><i class="fas fa-trash-alt"></i>Видалити</a>
                    <a class="a-left edit" href="<?php echo base_url .$part_url .'?edit&id='.$roww['id']; ?>"><i class="fas fa-pencil-alt"></i>Редагувати</a><br/><br/>
            ...
            if (isset($_GET['edit'])) {
                ?>
                <div class="sidebar2">
                    <form action="<?php echo base_url .$part_url .'?edit&id='.$roww['id']; ?>" method="post">

                        Заголовок:<br/>
                        <input type="text" name="title" value="<?php echo $roww['title']; ?>" /><br/><br/>

                        Текст: <br/>
                        <textarea class="ckeditor" name="text" cols="80" rows="20"><?php echo $roww['text']; ?></textarea><br/><br/>
