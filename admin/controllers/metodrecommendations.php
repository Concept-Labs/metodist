...
// страницу, чтобы избежать ошибки
        if ($page<1) {
            $page=1;
            header("Location: /metodrecommendations?page=1");
            exit();
        }
// Достаєм всі елементи із таблиці teacher і поміщаємо в таблицю metodrecommendations
        ...
        while ($row1 = mysqli_fetch_array($result1)) {
            ...
            $resultat1 = mysqli_query($db, "   SELECT * FROM metodrecommendations WHERE title='$title1' AND text='$text1' AND author='$author1'");
            ...
            if (empty($myrow1['id'])) {                    
                ...
                while ($row_del1 = mysqli_fetch_array($result_del1)) {
                    $id_del1 = $row_del1['id'];
                    $title_del1 = $row_del1['title'];
                    $text_del1 = $row_del1['text'];
                    $image_del1 = $row_del1['image'];
                    $author_del1 = $row_del1['author'];

                    $resultat_del1 = mysqli_query($db, "   SELECT * FROM teacher WHERE title='$title_del1' AND text='$text_del1' AND author='$author_del1'");

                    $myrow_del1 = mysqli_fetch_array($resultat_del1);

                    if (empty($myrow_del1['id'])) {                    
                        $delete1 = mysqli_query($db, "DELETE FROM `metodrecommendations` WHERE id='$id_del1'");
